...
class RequestId extends AbstractHeaderMiddleware
{
    ...
    protected function getValue(): ?string
    {
        ...
        // If no request ID is found in either header, generate a new UUID as the request ID
        if (Validator::isEmpty($requestId)) {
            $requestId = Uuid::generate();
        }

        // Add request ID information to the log context for debugging and tracing purposes
        Log::withContext(['technical-metadata' => ['request-id' => $requestId]]);
