        ...
        switch ($operator) {
            ...
            case 'exact':
                if ($this->connection->getBypassMapValidation()) {
                    ...
                    if (! $keywordField) {
                        throw new ParameterException(__("Field ['%1'] is not a keyword field which is required for the [exact] operator.", $field));
                    }
                }
            ...
            default:
                throw new QueryException(message: __("Invalid operator ['%1'] provided for condition.", $operator),  statusCode: 400);
        }
        ...
        if ($options) {
            foreach ($options as $key => $value) {
                switch ($key) {
                    ...
                    default:
                        throw new ParameterException(__('Unexpected option: %1', $key));
                }
            }
        }
