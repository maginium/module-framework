 ...
 */
class UpdateObserver extends AbstractObserver
{
    /**
    ...
    protected function handle(): void
    {
        try {
            ...
            // Step 2: Check if the payload is an array and call Firestore::update or Firestore::updateOne.
            $payload = $this->data->getPayload();

            if (Validator::isArray($payload)) {
                // If payload is an array, use Firestore::update
                Firestore::update(
                    $this->data->getTopic(), // Topic/channel to publish to.
                    $payload, // Data payload to send.
                );
            } else {
                // If payload is not an array, use Firestore::updateOne
                Firestore::updateOne(
                    $this->data->getTopic(), // Topic/channel to publish to.
