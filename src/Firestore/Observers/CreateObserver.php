...
class CreateObserver extends AbstractObserver
{
    ...
    protected function handle(): void
    {
        try {
            ...
            // Step 2: Check if the payload is an array and call Firestore::create or Firestore::createOne.
            $payload = $this->data->getPayload();

            if (Validator::isArray($payload)) {
                // If payload is an array, use Firestore::create
                Firestore::create(
                    $this->data->getTopic(), // Topic/channel to publish to.
                    $payload, // Data payload to send.
                );
            } else {
                // If payload is not an array, use Firestore::createOne
                Firestore::createOne(
                    $this->data->getTopic(), // Topic/channel to publish to.
