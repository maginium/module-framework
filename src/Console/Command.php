...
abstract class Command extends SymfonyCommand
{
     ...
     *
     * This method checks if the `MAGENTO_BIN` constant is defined. If not, it defaults to `DEFAULT_MAGENTO_BINARY`.
     *
     * @return string The path to the Magento binary.
     */
    protected static function magentoBinary(): string
    {
        // Return the defined MAGENTO_BIN constant or default to the constant value.
        return defined('MAGENTO_BIN') ? MAGENTO_BIN : self::DEFAULT_MAGENTO_BINARY;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            // Try to call the 'handle' method if it exists
            if (Reflection::methodExists($this, 'handle')) {
                $exitCode = $this->handle();
            }
            // Otherwise, try calling the '__invoke' method if it exists and is callable
            elseif (Reflection::methodExists($this, '__invoke') && is_callable([$this, '__invoke'])) {
                $exitCode = $this->__invoke();
            } else {
                // No executable method found; output an error message
                $this->line('<error>No executable method found.</error>');
            }
    ...
    public function callClosure($callback, array $parameters = [], $defaultMethod = null)
    {
        ...
        if (Validator::isString($callback) && str_contains($callback, '@')) {
            ...
            // Create the instance
            $instance = Container::create($class);
    ...
    {
        return Container::has(CommandMutex::class)
            ? Container::make(CommandMutex::class)
            : Container::make(CacheCommandMutex::class);
    }
    ...
    protected function invokeCallback($callback, array $parameters, $defaultMethod)
    {
        ...
        if ($defaultMethod && Validator::isString($callback)) {
            return Container::create($callback)->{$defaultMethod}(...$parameters);
        }
