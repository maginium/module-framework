...
/**
 * MongoStore class implements the LockableInterface interface for managing Mongo cache with locking capabilities.
 *
 * This class provides methods for interacting with a Mongo-based cache, including retrieving,
...
class MongoDbStore implements StoreInterface
{
    ...
    /**
     * Constructor for Mongo Store.
     *
     * Initializes the Mongo store with the provided Mongo instance, cache client,
     * lock factory, and an optional key prefix. This class also determines if the Mongo
     * instance is using version 3 or higher based on the number of parameters in the `getMulti` method.
     *
     * @param Mongo $mongo The Mongo instance used for caching operations.
    ...
    {
        // Fetch the value from the Mongo connection based on the provided key
    ...
    {
        // Use Mongo 'incrby' command to increment the value associated with the specified key
        // Increment the value by the given amount (or 1 by default if no value is provided)
        return $this->getMongo()->incrby($this->itemKey($key, true), $value);
    }
    ...
    {
        // Perform the decrement operation using the Mongo 'decrby' command
        // This will subtract the value from the current cache value at the specified key
        return $this->getMongo()->decrby($this->itemKey($key, true), $value);
    }
    ...
    {
        return $this->mongo->delete($this->prefix . $key);
    }
    ...
    {
        return $this->mongo->flush();
    }

    /**
     * Get the Mongo connection instance used for cache operations.
     *
    ...
    /**
     * Get the underlying Mongo connection.
     *
     * This method returns the Mongo client instance that is directly interacting
     * with the Mongo database. It can be used to perform Mongo-specific operations.
     *
     * @return Mongo The Mongo client instance.
     */
     ...
     * to avoid key collisions when the same Mongo instance is shared among multiple applications.
     *
